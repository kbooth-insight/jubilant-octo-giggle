...
resource "azurerm_network_security_group" "cvstacknsg" {
...
resource "azurerm_network_interface" "cvstacknic" {
    name                      = "myNIC"
    location = "${var.location}"
    resource_group_name       = "${azurerm_resource_group.cvstackgroup.name}"
...
resource "azurerm_virtual_machine" "cvstackconsulnode" {
    name                  = "box${count.index}"
    location = "${var.location}"
    resource_group_name   = "${azurerm_resource_group.cvstackgroup.name}"
    network_interface_ids = ["${azurerm_network_interface.cvstacknic.id}"]
